...
依頼文の構造化では、タイムアウト、接続失敗、HTTP 429、再試行可能な5xx、
不正JSON・スキーマ不一致だけを最大2回再試行する。待機時間には指数バックオフと
ジッターを適用する。認証失敗などの4xxは再試行しない。

構造化できなかった場合、APIは`mode: manual`、元の入力、手入力フォームの初期値、
安全な失敗分類を返す。入力は自動公開されず、ユーザーの編集・確認後に通常の
`POST /requests`へ送信する。手動投稿でも禁止作業の固定ルール検査は必須である。
障害分類別の件数とフォールバック件数をプロセス内メトリクスで保持し、ログには
分類・試行回数・再試行可否だけを記録する。依頼本文、外部APIの例外本文、APIキーは
ログへ記録しない。
