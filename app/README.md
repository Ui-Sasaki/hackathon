...
ブロック関係はブロックした利用者と対象利用者の組で保持する。どちらかが相手を
ブロックしている間、相手の依頼・応募・メッセージは表示されず、新規応募、応募者選択、
メッセージ送信は 403 となる。ブロック解除後は表示と操作が再び有効になる。

### 応募者選択・排他的マッチング

応募者選択は、版番号の検証、定員確認、応募・依頼・マッチの更新を単一の
クリティカルセクションで処理する。同時選択では先に成立した1件だけを確定し、
競合した操作には409を返す。本番のSupabase接続では、この境界をDB RPCの
トランザクションとして実装する。
