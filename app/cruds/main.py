...
# Register CORS last so it is the outermost middleware. SuperTokens handles
# /auth/* itself, and its responses must still receive credentialed CORS
# headers when the web app and API are hosted on different origins.
...
async def create_message(
    ...
    if isinstance(repository, MemoryMessageRepository):
        ...
        if is_blocked_pair(current_user.user_id, other_id):
            raise HTTPException(404, detail={"code": "MATCH_NOT_FOUND"})
    item = await repository.create(current_user, match_id, body.body)
    if item is None:
        raise HTTPException(404, detail={"code": "MATCH_NOT_FOUND"})
    return item
