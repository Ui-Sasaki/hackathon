...
begin
    ...
    v_result := app.send_match_message(v_match, '依頼者からのメッセージ');
...
begin
    ...
    if app.match_is_visible(v_match)
       or app.send_match_message(v_match, '送信不可') ->> 'code' <> 'MATCH_NOT_FOUND' then
...
do $$
begin
    if app.send_match_message(current_setting('test.match_id')::uuid, '部外者')
