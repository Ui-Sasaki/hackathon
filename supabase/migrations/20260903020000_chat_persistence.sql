...
create or replace function app.send_match_message(p_match_id uuid, p_body text)
...
begin
    ...
    values (p_match_id, app.current_actor(), p_body, now())
...
revoke all on function app.send_match_message(uuid, text) from public;
revoke all on function app.mark_match_messages_read(uuid) from public;
grant execute on function app.send_match_message(uuid, text) to tetote_app;
