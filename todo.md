...
状態はコード上の実装と利用箇所を基準にする。「クライアント済・画面未接続」はAPIを呼ぶTypeScript関数と単体テストはあるが、画面のユーザー操作から呼ばれていない状態を表す。DB欄の「Memoryのみ」はプロセス再起動でデータが失われ、本番永続化には未接続であることを表す。

| 機能 | バックエンドAPI | APIクライアント | 画面接続 | DB永続化 | TODO |
|---|---|---|---|---|---|
| プロフィール取得・更新 | 実装済み | 実装済み | 接続済み | Postgres対応済み | 04 |
| 概算地域解決 | 実装済み | 実装済み | 未接続（同意UI調整待ち） | 対象外（座標は保存禁止） | 08 |
| マスキングプレビュー | 実装済み | 実装済み | 接続済み | 対象外（元本文は保存禁止） | 09 |
| 依頼構造化 | 実装済み | 実装済み | 接続済み | 監査情報のみMemory | 10 |
| 依頼作成 | 実装済み | 実装済み | 未接続 | Postgres対応済み | 11 |
| 依頼一覧 | 実装済み | 実装済み | 未接続 | Postgres対応済み | 12 |
| 依頼詳細 | 実装済み | 実装済み | 未接続 | Postgres対応済み | 13 |
| 依頼更新 | 実装済み | 実装済み | 未接続 | Postgres対応済み | 14 |
| 依頼削除（取消） | 実装済み | 実装済み | 未接続 | Postgres対応済み | 15 |
| 応募作成 | 実装済み | 実装済み | 接続済み | Postgres対応済み | 16 |
| 応募辞退 | 実装済み | 実装済み | 接続済み | Postgres対応済み | 17 |
| 応募者一覧 | 実装済み | 実装済み | 未接続 | Postgres対応済み | 18 |
| 応募者選択 | 実装済み | 実装済み | 未接続 | Postgres原子操作対応済み | 19 |
| マッチ詳細 | 実装済み | 実装済み | 未接続 | Postgres対応済み | 20 |
| メッセージ一覧 | 実装済み | 未実装 | 未接続 | Postgres対応済み | 21 |
| メッセージ送信 | 実装済み | 未実装 | 未接続 | Postgres対応済み | 22 |
| メッセージRealtime | 未実装 | 未実装 | 未接続 | 未実装 | 23 |
| 完了確認 | 実装済み | 未実装 | 未接続 | Postgres原子操作対応済み | 24 |
| dispute | 実装済み | 未実装 | 未接続 | Postgres原子操作対応済み | 25 |
| レビュー投稿 | 実装済み | 未実装 | 未接続 | Memoryのみ | 26 |
| 実績生成 | モック実装済み | 未実装 | 未接続 | Memoryのみ | 27 |
| 実績公開範囲 | 実装済み | 未実装 | 未接続 | Memoryのみ | 28 |
| 本人確認申請 | モック実装済み | 未実装 | 未接続 | Memoryのみ・画像アップロード未実装 | 29 |
| 通報 | 実装済み | 未実装 | 未接続 | Memoryのみ | 30 |
| ブロック・解除 | 実装済み | 未実装 | 未接続 | Memoryのみ | 31 |

画面接続が完了した機能は、APIレスポンスを画面状態へ反映し、loading・empty・error・再試行・競合など必要な状態を接続テストで確認した時点で「接続・検証済み」へ更新する。APIクライアントの単体テスト成功だけでは画面接続完了としない。
